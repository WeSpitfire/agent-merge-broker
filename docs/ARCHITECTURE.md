...
Version `0.14.2` extends Gate with diagnostics, abandonment, journaled retirement, detached signed
...
In `0.14.2`, `state-codec.ts` validates the known nested task, batch, candidate, lease, validation,
...
indefinitely. In `0.14.2`, `SubmissionRetentionManager` owns their separate `candidate archive`
...
merge reconciliation, and dependency release remain outside Gate. Version `0.14.2` abandonment, retirement,
and detached evidence extend this aggregate without inventing Coordinate history.

### Internal boundaries and detached evidence — 0.14.2
