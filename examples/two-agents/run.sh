...
# Repository policy is committed, so every linked worktree sees the same rules. Keep the signing
# public key generated by `init` while adding demo-specific validators.
node --input-type=module <<'JS'
import { readFileSync, writeFileSync } from "node:fs";

const target = ".merge-broker/config.json";
const config = JSON.parse(readFileSync(target, "utf8"));
config.baseRef = "main";
config.leases.serializedPatterns = ["package-lock.json"];
config.policies.requireCleanWorktree = true;
config.integration.refreshBase = false;
config.validation.focused = [
  {
    name: "changed files exist",
    paths: ["src/**"],
    command: 'for file in {files}; do test -e "$file"; done',
    timeoutSeconds: 60,
  },
];
...
say "The final commit signs how the batch was assembled"
