...
program
  ...
  .option("--apply", "rewrite upgradable files; requires a complete scan with no unsupported or unreadable files")
  .action(async (options: { apply?: boolean }) => {
    ...
    output(report, [
      report.applied
        ...
        : !report.complete
          ? "Saved-format inspection is incomplete; compatibility with this release has not been established."
    ...
    if (!report.applied && (!report.complete || report.pending > 0 || report.blocked > 0)) process.exitCode = CLI_EXIT_CODES.rejected;
  });
