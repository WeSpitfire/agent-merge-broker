...
export class GitRepository {
  ...
  */
  ...
  /** Release only the direct broker ref at the expected OID; missing is a completed retry. */
  async releasePinnedLocalRef(pinId: string, expectedOid: string): Promise<void> {
    const ref = adoptedRef(pinId);
    const symbolic = await this.localObjectGit(["symbolic-ref", "-q", ref], this.root, true);
    if (symbolic.exitCode !== 1) {
      throw new BrokerError("SUBMISSION_REF_CHANGED", `Refused to release a symbolic or unreadable ref: ${ref}`);
    }
    const exists = await this.localObjectGit(["show-ref", "--verify", "--quiet", ref], this.root, true);
    if (exists.exitCode === 1) return;
    if (exists.exitCode !== 0) {
      throw new BrokerError("SUBMISSION_REF_RELEASE_FAILED", `Could not inspect retained ref ${ref}.`);
    }
    await this.assertPinnedLocalRef(pinId, expectedOid);
    const result = await this.withIsolatedHooks(async (hooksDirectory) => await this.localObjectGit([
      "-c", `core.hooksPath=${hooksDirectory}`,
      "update-ref", "--no-deref", "-d", ref, expectedOid,
    ], this.root, true));
    if (result.exitCode !== 0) {
      throw new BrokerError("SUBMISSION_REF_RELEASE_FAILED", `Could not release retained ref ${ref}.`, {
        stderr: result.stderr,
      });
    }
    const after = await this.localObjectGit(["show-ref", "--verify", "--quiet", ref], this.root, true);
    if (after.exitCode !== 1) {
      throw new BrokerError("SUBMISSION_REF_RELEASE_FAILED", `Retained ref ${ref} is not confirmed absent.`);
    }
  }
